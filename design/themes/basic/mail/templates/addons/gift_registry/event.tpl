...
<a href="{"events.unsubscribe?access_key=`$access_key`&email=`$recipient_email`"|fn_url:'C':'http'}">{__("unsubscribe")}</a><br /><br />
{else}
<a href="{"events.view?event_id=`$event.event_id`"|fn_url:'C':'http'}">{__("view_event_details")}</a><br />
<a href="{"events.unsubscribe?event_id=`$event.event_id`&email=`$recipient_email`"|fn_url:'C':'http'}">{__("unsubscribe")}</a><br /><br />
{/if}
